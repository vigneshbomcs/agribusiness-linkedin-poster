...
async function runOnGitHub() {
  ...
  const authJson = process.env.LINKEDIN_AUTH_JSON;
  const authFile = path.join(__dirname, 'linkedin_auth.json');

  if (authJson) {
    console.log('Restoring complete authenticated session from LINKEDIN_AUTH_JSON secret...');
    fs.writeFileSync(authFile, authJson, 'utf8');
  }

  if (!fs.existsSync(authFile)) {
    console.error('ERROR: No authentication state found! Please provide LINKEDIN_AUTH_JSON secret.');
    process.exit(1);
  }
  ...
  console.log('\n[1/4] Launching headless browser with full storageState...');
  ...
  console.log('[2/4] Navigating to LinkedIn feed...');
